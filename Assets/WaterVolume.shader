Shader "Custom/Water" {
    Properties {
        ...
        // Cut (object-space Y)
        _CutY ("Cut Height (Object Y)", Float) = 0.0
        _CutFeather ("Cut Feather (Object Y)", Float) = 0.02
    }
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput Vert (VertexInput input) {
                ...
                // object-space Y for masking + clipping
                float objY = input.positionOS.y;

                float3 worldPos = mul(unity_ObjectToWorld, input.positionOS).xyz;

                // Smooth mask: 0 below cut, 1 above cut (with a tiny feather)
                float waveMask = smoothstep(_CutY, _CutY + max(1e-5, _CutFeather), objY);

                // Only add waves where mask > 0 (top region)
                worldPos.y += WaveHeight(worldPos.xz, t) * waveMask;
            ...
            float4 Frag (VertexOutput input) : SV_Target {
                // Hard cut in object-space Y
                clip(input.objectY - _CutY);
